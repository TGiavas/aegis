...
from app.db import AsyncSessionLocal, Alert
from app.rules import evaluate_event, AlertTrigger
...
async def process_message(message: AbstractIncomingMessage) -> None:
    ...
    async with message.process():
        try:
            ...
            print(f"📥 Processing event: {event.get('event_type')} from {event.get('source')}")
            
            # Evaluate rules
            triggers = evaluate_event(event)
            ...
            if triggers:
                async with AsyncSessionLocal() as db:
                    for trigger in triggers:
                        ...
                        print(f"🚨 Alert created: [{trigger.level}] {trigger.rule_name}")
